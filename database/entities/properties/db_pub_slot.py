...
class DbPubSlot(object):
    ...
    def create(self, name):
        ...
        task_pub_slot_db_path = DbPaths.origin_path(DbPaths.db_task_pub(), name)
    ...
    def add(self, pub_slot=[]):
        for each in pub_slot:
            task_imports_from_address = DbPaths.origin_path(DbPaths.db_task_pub(), each)
    ...
    def add_dict(self, pub_slot=[]):
        ...
        for each in pub_slot:
            ...
            pub_slot_path = DbPaths.origin_path(DbPaths.db_task_pub(), get_slot_name[0])
            DbUpdate().origin_update(asset_id, pub_slot_path, get_slot_param[0])
        print("Publish Slot added succesfully!")

    def get_pub_slots(self):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "pub_slots")
            cursor = self.db[DbProject().get_branch_type]
    ...
    def get_type(self, pub_slot):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "pub_slots", pub_slot, "type")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def get_method(self, pub_slot):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "pub_slots", pub_slot, "method")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def get_used_by(self, pub_slot):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "pub_slots", pub_slot, "used_by")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def get_is_reviewable(self, pub_slot):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "pub_slots", pub_slot, "reviewable")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def get_is_active(self, pub_slot):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "pub_slots", pub_slot, "active")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def set_used_by(self, pub_slot, used_by, remove_action=False):
        cursor = self.db[DbProject().get_branch_type]
        pub_slot_path = DbPaths.origin_path(DbPaths().db_task_pub(), pub_slot, "used_by")
        asset_id = DbIds.db_entry_id()
