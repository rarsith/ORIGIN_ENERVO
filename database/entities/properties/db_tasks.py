...
class DbTasks(object):
    ...
    def create(self, name):
        ...
        task_db_path = DbPaths.origin_path(DbPaths.db_task_path(), name)
    ...
    def get_definition(self, task=Envars.task_name):
        ...
        try:
            task_path = DbPaths.origin_path("tasks", task)
            cursor = self.db[DbProject().get_branch_type]
    ...
    def get_imports_from(self):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "imports_from")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def get_status(self):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "status")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def get_is_active(self):
        try:
            task_path = DbPaths.origin_path("tasks", Envars.task_name, "active")
            cursor = self.db[DbProject().get_branch_type]
            tasks_list = cursor.find({"_id": DbIds.db_entry_id()}, {'_id': 0, task_path: 1})
            for tasks in tasks_list:
    ...
    def set_imports_from(self, imports_from=[]):
        for each in imports_from:
            task_imports_from_address = DbPaths.origin_path(DbPaths.db_task_imp_from(), each)
